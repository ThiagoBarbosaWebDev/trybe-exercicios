...
const handleDragonDamage = () => dragon.damage = Math.floor(Math.random() * (dragon.strength - 15 + 1) + 15);

const handleWarriorDamage = () => warrior.damage = Math.floor(Math.random() * ((warrior.weaponDmg * warrior.strength) - warrior.strength + 1) + warrior.strength);

const handleMageDamage = () => {
  if (mage.mana < 15) {console.log('Não possui mana suficiente'); return mage.damage = 0;} else {
    return Math.floor((Math.random() * ((mage.intelligence * 2) - (mage.intelligence + 1))) + mage.intelligence);
  }
}

const  handleMageManaCost = () => {
  if (mage.mana <= 15) {return 0} else {
    return 15;
  }
}
...
const handleMageTurn = () => {
  ...
  dragon.healthPoints -= mageTurn.dano
  mage.mana -= mageTurn.manaCost
  mage.damage = mageTurn.dano
  console.log(`Mage casts a magic missiles for ${mageTurn.dano} points of force damage`)
}

const handleWarriorTurn = () => {
  warriorHit = handleWarriorDamage()
  warrior.damage = warriorHit
  dragon.healthPoints -= warriorHit
  console.log(`Warrior hits the Dragon with his broadaxe for ${warriorHit} points of slashing damage`)
}

const handleDragonTurn = () => {
  dragonHit = handleDragonDamage()
  dragon.damage = dragonHit
  mage.healthPoints -= dragonHit
  warrior.healthPoints -= dragonHit
  console.log(`The dragon breathes fire! All party members are hit for ${dragonHit} points of fire damage`)
}

const gameActions = {
  // Crie as HOFs neste objeto.
    warriorTurn: () => handleWarriorTurn(),
    mageTurn: () => handleMageTurn(),
    dragonTurn: () => handleDragonTurn(),
    turnReport: () => {
